package com.jomardev25.ecommerce.searchservice.service.impl;
...
import com.jomardev25.ecommerce.searchservice.util.Permutation;
...
public class SearchSuggestionServiceImpl implements SearchSuggestionService {
		...
		while (true) {
			 ...
			 try {
				...
				if (response.statusCode() == 200) {
                    ...
                    constructAndAddKeywordCombination(jsonResponse, "genderApparelKeywords", new String[]{"genders", "apparels"});
                    constructAndAddKeywordCombination(jsonResponse, "genderBrandKeywords", new String[]{"genders", "brands"});
                    constructAndAddKeywordCombination(jsonResponse, "apparelBrandKeywords", new String[]{"apparels", "brands"});
                    constructAndAddKeywordCombination(jsonResponse, "threeAttrKeywords", new String[]{"genders", "apparels", "brands"});
					...
					System.out.println("Error: Unable to connect Search Suggestion API status code = " + response.statusCode());
				}
	...
	private void addJsonObjKeywordToMap(JSONObject jsonResponse, String key, String attributeName) throws JSONException {
        ...
        for (int index = 0; index < jsonArray.length(); ++index) {
            ...
            addSearchSuggestionKeywords(jsonObject.getString("type"), filterLink.append(attributeName).append("=").append(jsonObject.getString("id")));
            if (index < 10 && attributeName.equals("apparels")) {
                defaultSearchSuggestionList.add(new SearchSuggestionKeywordInfo(jsonObject.getString("type"), filterLink, 1));
            }
        }
    }
