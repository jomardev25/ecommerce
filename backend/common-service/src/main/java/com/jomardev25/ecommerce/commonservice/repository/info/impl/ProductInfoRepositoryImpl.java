package com.jomardev25.ecommerce.commonservice.repository.info.impl;
...
import com.jomardev25.ecommerce.commonservice.repository.info.queryhelpers.ProductQueryHelper;
import com.jomardev25.ecommerce.commonservice.repository.info.queryhelpers.context.ParamsToQueryContext;
import com.jomardev25.ecommerce.commonservice.utils.resulttransformers.ListResultTransformer;
...
public class ProductInfoRepositoryImpl {
    ...
    public Pair<Long, List<ProductInfo>> getProductsByCategories(HashMap<String, String> conditionMap) {
		...
		TypedQuery<Long> totalCountQuery = (TypedQuery<Long>) entityManager.createQuery("select count(*) from ProductInfo p where " + String.join(" AND ", conditions));
        ...
        if (totalCountQueryResultList != null && totalCountQueryResultList.get(0) > 0) {
            ...
            TypedQuery<ProductInfo> query = entityManager.createQuery("select p from ProductInfo p where " + String.join(" AND ", conditions) + sortBy, ProductInfo.class);
    ...
    private ParamsToQueryContext filterAndGetConditionMap(ProductQueryHelper productQueryHelper, HashMap<String, String> conditionMap, String queryParam) {
    ...
    private BrandsAndApparelsDTO getBrandsAndApparelsList(int gender_id) {
        ...
        brandsAndApparelsDTO.setBrands(listResultTransformer.getFilterAttributesResultTransformer(
                        ...
                        " ", mapParams, entityManager)); // " order by count(*) desc ", mapParams, entityManager));
